	...
	public static void edgeDetection(int[] imageData, int width) {
		...
		for (int y = 0; y < height; y++) //Iterate through each row
		{
			for (int x = 0; x < width; x++) //Iterate through each column
			{
				...
				//Combine horizontal and vertical color channels
				...
				//Write max of R, G, B value into edges array
		...
		{
			//If edge value meets threshold, replace pixel of imageData with white
			if(edges[i] > edgeThreshold)
				imageData[i] = 0xFFFFFF;
			else //Otherwise pixel will be replaced with black
